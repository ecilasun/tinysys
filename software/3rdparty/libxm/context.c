...
#define OFFSET(ptr,thetype) do {										\
		(ptr) = (thetype*)((intptr_t)(ptr) + (intptr_t)(*ctxp));	\
	} while(0)
...
void xm_create_context_from_libxmize(xm_context_t** ctxp, char* libxmized, uint32_t rate) {
	...
	*ctxp = (xm_context_t*)libxmized;
	...
	OFFSET((*ctxp)->module.patterns, xm_pattern_t);
	OFFSET((*ctxp)->module.instruments, xm_instrument_t);
	OFFSET((*ctxp)->row_loop_count, uint8_t);
	OFFSET((*ctxp)->channels, xm_channel_context_t);

	for(i = 0; i < (*ctxp)->module.num_patterns; ++i) {
		OFFSET((*ctxp)->module.patterns[i].slots, xm_pattern_slot_t);
	}

	for(i = 0; i < (*ctxp)->module.num_instruments; ++i) {
		OFFSET((*ctxp)->module.instruments[i].samples, xm_sample_t);

		for(j = 0; j < (*ctxp)->module.instruments[i].num_samples; ++j) {
			OFFSET((*ctxp)->module.instruments[i].samples[j].data8, int8_t);
