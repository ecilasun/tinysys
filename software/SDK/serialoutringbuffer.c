/**
 * @file serialoutringbuffer.c
 * 
 * @brief Serial output buffer implementation
 * 
...
volatile uint32_t *m_so_readOffset  = (volatile uint32_t*)(SEROUT_RINGBUFFER_STATE+4);
volatile uint32_t *m_so_writeOffset = (volatile uint32_t*)(SEROUT_RINGBUFFER_STATE+8);

/**
 * @brief Reset the ring buffer to empty state
 */
void __attribute__ ((noinline)) SerialOutRingBufferReset()
{
    *m_so_readOffset  = 0;
    *m_so_writeOffset = 0;
}
 ...
 */
uint32_t __attribute__ ((noinline)) SerialOutRingBufferRead(void* pvDest, const uint32_t cbDest)
{
    uint8_t *ringbuffer = (uint8_t *)SEROUT_RINGBUFFER_BASE;

    uint32_t readOffset = *m_so_readOffset;
    const uint32_t writeOffset = *m_so_writeOffset;
