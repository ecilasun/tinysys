#include "tbm_printf.h"
 ...
 TYPE tbm_round(TYPE value)
 {
	 uint64_t integer_part = (uint64_t)value;
	 TYPE rest = (value - integer_part);
	 if (value > 0) {
		 if (rest >= 0.5f)
			 return (TYPE)integer_part + 1.0;
	 }
	 else {
		 if (rest <= -0.5f)
			 return (TYPE)integer_part - 1.0;
	 }
	 return (TYPE)integer_part;
 }
 ...
 inline int tbm_ulltoa(uint64_t value, char* buf, size_t size, size_t base = 10, const char* digits = "0123456789abcdef") {
 ...
 template<typename TYPE>
 inline int tbm_ftoa(TYPE value, char* buf, size_t size, int precision) {
	 if (size < 1)
		 return -1;
	 ...
	 pos += tbm_ulltoa(integer_part, &buf[pos], size - pos);
	 if (pos >= size)
		 return -1; // check for buffer overflow
	 ...
	 if (precision > 0) {
		 ...
		 if (pos >= size)
			 return -1; // check for buffer overflow
		 pos += tbm_ulltoa(fractional_part, &buf[pos], size - pos);
		 if (pos >= size)
			 return -1; // check for buffer overflow
	 }
